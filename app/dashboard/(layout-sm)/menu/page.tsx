...
function TermsLink(item: MenuType) {
  return (
    <Link href={item.link}>
      <Button variant="link" className="underline text-black tracking-wider px-0">{item.label}</Button>
    </Link>
  )
}
...
export default function MenuPage() {
  return (
    <>
      {menus.map((menu, idx) => <MenuItem key={`menu-page-menu-item-${idx}`} {...menu} />)}

      <Image src={finans_image} alt="finans-logo" width={150} height={150} className="py-2" />
